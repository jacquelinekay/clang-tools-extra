...
if __name__ == '__main__':
    ...
    for peripheral in device_properties.find_all('peripheral'):
        for register in format_utils.get_registers(peripheral):
            ...
            cur_reg['address'] = format_utils.sanitize_int(str(register.addressOffset.string), 0)
            cur_reg['name'] = format_utils.format_register_name(peripheral, register).encode('utf8')
            if len(register.find_all('field')) > 0:
                cur_reg['fields'] = []
    ...
    with open(output_filename, 'w+') as f:
        ...
        output = yaml.dump_all(register_map, f, explicit_start=True, canonical=False, default_flow_style=False, Dumper=yaml.Dumper, encoding='utf8')
